...
def fetch_transcript(video_id):
    ...
    try:
        ...
        if not text:
            return {"status": "no_transcript", "length": 0}
        return {"status": "success", "length": len(text)}
    except urllib.error.HTTPError as e:
        return {"status": "http_error", "httpCode": e.code}
...
def main():
    if not VIDEOS_FILE.exists():
        raise RuntimeError(
            "data/videos.json がありません。先に update_videos.py を実行してください。"
        )
    ...
    # 未確認、または一時エラーの動画を処理する。
    # success / no_transcript は再確認しない。
    pending = [
        video
        for video in videos
        if (
    ...
    for index, video in enumerate(selected, start=1):
        ...
        print()
        print(f"[{index}/{len(selected)}] {channel} / {title}")
